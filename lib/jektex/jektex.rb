...
def get_list_of_updated_global_macros(current_macros, cached_global_macros)
  return [] unless cached_global_macros || current_macros
...
def ignored?(page)
  ...
  ignored.any? { |patern| File.fnmatch?(patern, page.relative_path, File::FNM_DOTMATCH) }
end

def contains_updated_global_macro?(expression)
  updated_global_macros.any? { |m| expression[m] }
end
...
def render_latex_notation(page)
  ...
  post = post.gsub(REGEXP_1) do |m|
    escape_method($1, $2, page.relative_path)
  end

  # render display mode expressions
  post = post.gsub(REGEXP_2) do |m|
    escape_method($1, $2, page.relative_path)
  end
end
...
def escape_method(type, expression, doc_path)
  ...
  if (cache.has_key?(expression_hash) && !contains_updated_global_macro?(expression))
    # check if expression conains updated macro
  ...
  # else generate one and store it
    ...
    begin
      ...
      # save cache to disk
      File.open(path_to_cache, "w") do |to_file|
        Marshal.dump(cache, to_file)
      end
...
Jekyll::Hooks.register :site, :after_init do |site|
  ...
  if File.exist?(path_to_cache)
    cache = File.open(path_to_cache, "r") do |from_file|
  ...
  if config.has_key?("macros")
    config["macros"].each do |macro_definition|
